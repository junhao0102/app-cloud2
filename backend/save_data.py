...
def read_config(sql_code: str, create, logger):
    ...
    msg = {'host': config['DB_path']['host'],
           'database': config['DB_path']['database'],
           'user': config['DB_path']['user'],
           'password': config['DB_path']['password'],
           'sql': sql_code, 'port': config['DB_path']['port'],
           'create': create, 'logger': logger}
    return msg
...
def link_Postgres(
        ...
        sql: str, port: str, create, logger):
    try:
        conn = psy.connect(
        ...
        logger.info(
            f'[Error] {msg}; {file_name}. {func_name} at line {line_num}.')
...
def create_main(logger):
    # 新增資料表表單
    create_list = [
        'repair_table', 'machine_table', 'part_table',
        'part_statistic_table', 'machine_state_table',
        'create_huan_jia_table', 'create_part_list',
        'create_device_table', 'create_machine_list'
    ]
    save_msg = []
    try:
        check = 0
...
def init_part_data(logger):
    name = ['儲紗輪(第一段)', '儲紗輪(第二段)', '儲紗輪(第三段)', '儲紗輪(第四段)',
            '送紗皮帶(第一段)', '送紗皮帶(第二段)', '送紗皮帶(第三段)', '送紗皮帶(第四段)',
            '羅拉捲布皮帶', '馬達皮帶', '主機傳動皮帶', '捲布齒輪']
    init = {}
    build_msg = {}
    build_num = 0
    ...
    if db_msg['state']:
        for device_id in id_list:
            ...
            if df_dict['status'] == 'ok':
                ...
                error_msg = {'message': '[Error] input status does not ok or other input error', 'state': False}
...
def read_machine_state(logger):
    try:
        ...
        if db_msg1['state']:
            ...
            data = {'machine_name': m1_data['machine_name'].tolist(),
...
def update_machine_table(up_dict: Dict, logger):
    ...
    if sql != "N":
        ...
        msg_dict = {'message': f'[Error] input target ({up_dict["update_target"]}) error. check the update target',
                    'state': False}
        logger.info(f'[Error] input target ({up_dict["update_target"]}) error. check the update target')
    return msg_dict
